...
## 2026-08-26 — Prompt 7 follow-up — strip removed everywhere — nudges only on face

Owner, verbatim: "those need to be removed from all the other pages/dashboards as well. only have them popup on the family db the way you just set it up - visible nowhere else." The OriginsStrip banner now renders on NO route: `<OriginsStrip />` unmounted from `app/layout.tsx` (its only mount, global). Height reclaimed with ONE mechanism: a bare `:root { --strip-h: 0px }` appended at the very end of `globals.css` — later source order beats the committed 72px base and the committed 768px 72px tier at every width; every surface's existing `calc(… - var(--strip-h))` subtraction (`.dashboard`, `/board`, `/week`, `/money`, `/business`, `/table`, `/budget`, `/origins` — all inline math untouched) reads the same var, so all agree by construction. Dead wiring deleted: the appended `:root[data-strip-compact]` rules (both) and `.dashboard-nostrip` rule removed from globals.css; the `dashboard-nostrip` class dropped from `app/page.tsx`; the attribute effect removed from `OriginsStrip.tsx` (file kept, retired-marked — `OriginsNudges` still imports `deliverableOf`). The two committed `:root:has(…)` rules stay untouched (permanently unmatched), comments corrected. CornerStack/OriginsNudges/GoalsIntermission byte-untouched; they mount only via `app/page.tsx`.

Files: `app/layout.tsx`, `app/page.tsx`, `app/globals.css` (appended zero rule + deleted own Prompt-7 rules + comment corrections; tokens untouched), `app/components/OriginsStrip.tsx`, this ledger.

Measured (local prod build port 4123, headless Chromium, all `/api` GETs proxied to real production payloads, non-GET aborted — no write possible; 26/26 PASS): zero `.origins-strip` and computed `--strip-h` 0px on `/` `/money` `/business` `/table` `/board` `/week` @1920x936. Face floors unchanged from Prompt 7 (face already had 0px locally): lowest in-flow leaf 89.0/87.0/98.5px v1|2|3 @936 and @923, 95.0/93.0/104.5 @1080, scroll 0/0, no `$0.00`. `/board` + `/week` at all three heights: page scroll 0/0, per-card last-row geometric clip check clean, WeekProgressStrip present on `/board` (Ansar view — renders only for the selected person, unchanged behaviour). Corner stack: `page.clock` 12:35 Sydney → 1 `.corner-stack`, 2 `.onudge` on `/`; zero on `/money` and `/board`. 390x844 hscroll 0 on `/` and `/money`. `npx tsc --noEmit` + `npm run build` pass.

Not proven: production state (nothing committed/staged/deployed per instruction); Flip Pro rendering; the `/origins` and `/budget` pages re-measured only by construction (same var, taller scrollable pages — not fixed-viewport); real wall-clock nudge firing (faked clock).

## 2026-08-26 — Prompt 7 amendment — SILENT nudge no longer pins

Owner, verbatim: "we cant have them 'stay until ticked' let them sit idle for 60sec and fade away. use the same timers as the notion popups." SILENT pinning removed from `OriginsNudges.tsx` (only file with logic changes; CornerStack/GoalsIntermission untouched — the timer never lived there): every nudge card, SILENT included, now auto-dismisses after the ONE shared `AUTO_DISMISS_MS` (60 000 ms — the constant the windowed cards already used; no parallel 60000 added) and leaves with the same hard-cut exit the mission card has (both render null — no fade exists on either, consistency kept). A dismissed SILENT card returns an hour later (`SILENT_RETURN_MS`, in-memory `silentHiddenUntil` ref — same reload semantics as the windowed `spent` set); Later's persisted hourly snooze unchanged; hourly-return recurrence kept per the directive. Hint text: "stays until ticked" replaced by "hides in 60s · back hourly", value derived from `AUTO_DISMISS_MS` so it cannot drift. Note for the record: the mission card's own dwell is the settings-tunable `INTERMISSION_HOLD_MS` (default 10 s) — the owner's explicit "60sec" was taken as authoritative over literally importing that 10 s hold; if one shared knob is wanted later, that is a deliberate follow-up.

Files: `app/components/OriginsNudges.tsx`, this ledger. (Working tree still carries the uncommitted "strip removed everywhere" change set — left intact.)

Measured (local prod build port 4123, headless Chromium, `page.clock` faked at Sydney 15:00 — outside all windows; synthetic SILENT payload for taylan, nihal onpace; `/api/origins|mission|cycle` synthetic, other GETs proxied read-only to production, ALL non-GET aborted — no write possible; 5/5 PASS): mission card + silent nudge stack simultaneously, boxes non-intersecting (480×104.5 above 480×148.75, both x 1416, 24 px corner anchor); "hides in 60s" rendered, zero "stays until ticked"; card still present at ~+50 s, DOM count 0 just past 60 s (hard cut, no pinned card remains); still absent at +31 min; back at +62 min (hourly recurrence). `npx tsc --noEmit` + `npm run build` pass.
...
## 2026-08-26 — Prompt 8 — start paused, play to rotate

Owner, verbatim: "Ensure that when it loads it doesnt auto go to the next slide. On start up it stays on slide 1 - and only moves through slides if 'play' is pressed." The face `/` now MOUNTS PAUSED on frame 1: in `FaceRotator.tsx` the owner-pause state and its ref initialize `true` instead of `false` (three-line change plus comments; no persistence — every fresh load starts paused), and the paused aria-label became "Start rotation". Class semantics unchanged and reported: **`face-pause--on` present = paused (pill lit cyan, ▶ "Start rotation"); absent = playing (⏸ "Pause rotation")**. Everything downstream is the untouched Prompt-6 machinery: once playing, 10/15/25 dwell, hard cuts, 60 s touch-pause → resume at frame 1, visibilitychange resume at frame 1 (only while in playing mode — the existing `pausedRef` guard already keeps a never-started face paused across tab switches); pill pause/resume toggling as before; `?view=1|2|3` freeze + pill-hidden unchanged. This is a deliberate deviation from RESTRUCTURE-SPEC §7 "`/` rotates" by owner directive; spec not edited (scope changes to it are their own act).

Files: `app/components/face/FaceRotator.tsx`, this ledger. No CSS needed — existing `.face-pause--on` / `.face-counter--dim` styling covers the mounted-paused look.

Measured (local prod build port 4123, headless Chromium 1920x936, all non-GET aborted — no write possible; 21/21 PASS): fresh `/` → counter `1/3`, pill 48×48 with `face-pause--on` + aria "Start rotation", progress fill static (no `animate`, width 0) and still `1/3` after 16 s (past the 10 s dwell); 12 `/api` GETs in the mount batch then **0** further in 16 s (one shared load, no storm), frame 1 renders data while paused. Pill transitions: play → class off/aria "Pause rotation"/fill animating → `2/3` at +12 s; pause → class on, counter frozen `2/3` through 16 s; play again → resumes at `1/3` → `2/3` at +11 s. `?view=1` @1920x936: lowest leaf 37.0px from bottom (25–120 band), page scroll 0/0, pill absent, counter frozen `1/3` through 11 s. `npx tsc --noEmit` + `npm run build` pass.

Not proven: production state (nothing committed/staged/deployed per instruction); Flip Pro rendering; heights 1905x923/1920x1080 not re-measured this prompt (no layout change — state init only; 936 spot check held).
