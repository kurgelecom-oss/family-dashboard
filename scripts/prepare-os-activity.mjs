...
try{
 await db.begin(async tx=>{
  await tx.unsafe(await fs.readFile(new URL('../db/002-os-activity.sql',import.meta.url),'utf8'));
  await tx`insert into fds_os_tracking(workspace,member) values(${workspace},'nihal') on conflict do nothing`;
 });
