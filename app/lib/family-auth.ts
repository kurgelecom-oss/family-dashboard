...
export function isFamilyService(request:Request){const key=request.headers.get('x-family-edit-key');return !!key&&!!process.env.FAMILY_EDIT_KEY&&same(key,process.env.FAMILY_EDIT_KEY);}
...
export async function canEditFamily(request:Request){
 if(isFamilyService(request))return true;
