...
type BudgetEntry = { category: string; amount: number };
    ...
    <div className="panel col-4">
      ...
      <div className="stat-grid stat-grid-2" style={{ flex: "0 0 auto" }}>
        <div className="stat-cell">
          <div className="stat-num lg">${loading ? "—" : totalSpent.toFixed(0)}</div>
        ...
        <div className={`stat-cell ${remaining < 0 ? "alert-red" : "alert-green"}`}>
          <div className={`stat-num lg ${remaining < 0 ? "red" : "green"}`}>
            {loading ? "—" : `${remaining < 0 ? "-" : ""}$${Math.abs(remaining).toFixed(0)}`}
          </div>
        ...
        <div style={{ marginBottom: 10 }}>
          <div className="progress-row">
            ...
            <span className="num-label">
              {loading ? "—" : `${Math.min(Math.round((totalSpent / WEEKLY_TOTAL_TARGET) * 100), 100)}%`}
            </span>
          </div>
