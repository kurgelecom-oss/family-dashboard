...
const logDayLabel=(iso:string)=>new Intl.DateTimeFormat('en-AU',{weekday:'long',day:'numeric',month:'short',timeZone:'UTC'}).format(new Date(iso+'T12:00:00Z'));
...
function MetricRow({metric:m,onEdit}:{metric:Metric;onEdit:()=>void}){
 ...
 return <div className="su-metric" data-metric={m.key} title={m.detail}>
  <div><span className="su-metric-name">{m.key==='discoveries'&&m.href?<a className="su-source-link" href={m.href} target="_blank" rel="noreferrer" aria-label="Products found · open Nihal’s Notion surface log">{m.label} ↗</a>:m.label}</span><small title={m.detail}>{m.source==='missing'&&m.manualValue!==null?`${m.manualValue} extra saved · connected total unavailable`:m.backupIgnored?'Connected again · backup excluded':m.source==='manual'?'Manually recorded':m.source==='mixed'?`Connected + ${m.manualValue} manual`:m.detail}</small>{m.daily&&<div className="su-os-days" aria-label={m.key==='discoveries'?'Products found by day':'Nihal OS opens by day'}>{m.daily.map((d,i)=><span key={d.date} aria-label={`${DAY_NAMES[i]}: ${d.count??'not tracked'}`} title={`${DAY_NAMES[i]} ${dateLabel(d.date)}: ${d.count??'not tracked'}`}><small>{DAY_NAMES[i].slice(0,3)}</small><b>{d.count??'—'}</b></span>)}</div>}</div>
  ...
  {m.entries&&m.entries.length>0&&<details className="su-product-log"><summary>View {m.entries.length} surface {m.entries.length===1?'record':'records'}</summary><ul>{m.entries.map(e=><li key={e.id}><a href={e.href} target="_blank" rel="noreferrer">{e.name} ↗</a><time dateTime={e.timestamp??e.date}>{logDayLabel(e.date)} · {e.timestamp?timeLabel(e.timestamp):'time not recorded'}{e.dateSource==='Created time'?' · created':''}</time></li>)}</ul><a className="su-source-link" href={m.href} target="_blank" rel="noreferrer">Open surface log in Notion ↗</a></details>}
 </div>;
}
...
export default function SundayReport({theme='gather',preview=false}:{theme?:Theme;preview?:boolean}){
 ...
 return <main className={`su-root su-${theme}`}>
  ...
  <footer className="su-footer"><p><span className="su-live-dot"/> {error?'Last loaded':'Updated'} {timeLabel(report.generatedAt)} <span className="su-footer-divider"/> {allQuran?`${totalDays} Quran days between us`:'Quran feed needs a refresh'} <span className="su-footer-divider"/> <a href={report.sources.find(s=>s.id==='products')?.href} target="_blank" rel="noreferrer">{report.shared.discoveries??'—'} products surfaced in Nihal’s Notion log ↗</a></p><div><button type="button" onClick={()=>setSources(!sources)} aria-expanded={sources}>{connected}/{report.sources.length} sources</button><button type="button" onClick={refresh} disabled={loading}>{loading?'Refreshing…':'Refresh data'}</button></div></footer>
